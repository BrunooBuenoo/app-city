...
export async function votar(
  ...
  if (votoAnterior === tipo) {
    ...
    if (tipo === "concordo") {
      concordos--;
      deltaPontos = -RECOMPENSAS.CONCORDAR; // perde pontos se desfizer o "concordo"
    } else {
      discordos--;
    }
  } else {
    // Desfaz voto anterior se existir
    if (votoAnterior === "concordo") {
      concordos--;
    ...
    if (tipo === "concordo") {
      concordos++;
      deltaPontos += RECOMPENSAS.CONCORDAR; // ganha pontos ao concordar
    } else {
      discordos++;
    }
  }
